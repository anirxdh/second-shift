...
Generated 2026-09-13 15:07 by `uv run python -m evals.run`.
...
|---|---|---|---|---:|
| Planning | `full_day_callout` | A full-day call-out produces a legal plan that covers every job it can and is honest about the rest | PASS | 16 |
| Planning | `beats_greedy_baseline` | Finds chain moves a greedy dispatcher misses (move Wei's job to Ana so Wei can cover Marco) | PASS | 14 |
| Planning | `running_late` | A tech running late gets the smallest possible change: same jobs, shifted inside windows, no customer emails | PASS | 8 |
| Planning | `partial_day` | Partial-day absence: morning jobs move, the afternoon stays with the original tech | PASS | 12 |
| Planning | `two_out` | Two techs out: still legal, never invents coverage for skills nobody has | PASS | 5 |
| Planning | `deterministic` | Deterministic: the same input gives the same plan every time (repeatable demo and audits) | PASS | 62 |
| Planning | `calendar_busy_block` | A personal event on a tech's calendar is respected as busy time | PASS | 16 |
| Execution | `happy_path_execution` | Happy path writes everything once and the read-back verifies every write | PASS | 16 |
| Execution | `partial_with_uncoverable_job` | Partial-day call-out where a job can't be covered still executes cleanly end to end | PASS | 9 |
| Execution | `second_callout_same_day` | A second call-out later in the day keeps the first one in effect (never hands jobs back to Marco) | PASS | 34 |
| Execution | `double_approve` | Double-clicking Approve changes nothing the second time | PASS | 16 |
| Execution | `stale_plan_blocked` | If Calendar changes between plan and approval, nothing is written | PASS | 14 |
| Execution | `slack_rate_limit` | Slack rate limits are retried with backoff; no duplicate messages | PASS | 15 |
| Execution | `calendar_transient` | Calendar 5xx errors are retried; each job still booked exactly once | PASS | 15 |
| Execution | `crash_after_calendar_write` | Crash right after a Calendar write: resume finishes without double-booking | PASS | 15 |
| Execution | `crash_after_email` | Crash right after sending an email: resume does not email the customer twice | PASS | 20 |
| Execution | `crash_after_slack` | Crash right after a Slack post: resume does not post twice | PASS | 16 |
| Execution | `permanent_error_is_honest` | A permanent error stops the run and reports it; it never claims success | PASS | 14 |
...
| Guardrails | `callout_to_plan` | Clear call-out from the tech themself turns into a plan awaiting approval (no writes yet) | PASS | 13 |
...
| What-if | `whatif_from_slack` | 'What if Wei doesn't show up?' in Slack gets a simulated answer, and nothing changes | PASS | 14 |
| What-if | `whatif_adopt_then_execute` | 'Make it real' turns a what-if into a normal plan that executes and verifies | PASS | 29 |
| What-if | `preparedness_scan` | The preparedness scan simulates every call-out and finds the single points of failure | PASS | 78 |
...
- PASS reply: What-if: if Wei Chen is out, the plan covers 1 of 2 of their jobs; Elena Duarte would need...
