...
Generated 2026-09-13 12:10 by `uv run python -m evals.run`.
...
|---|---|---|---|---:|
| Planning | `full_day_callout` | A full-day call-out produces a legal plan that covers every job it can and is honest about the rest | PASS | 36 |
...
| Planning | `deterministic` | Deterministic: the same input gives the same plan every time (repeatable demo and audits) | PASS | 61 |
| Planning | `calendar_busy_block` | A personal event on a tech's calendar is respected as busy time | PASS | 16 |
| Execution | `happy_path_execution` | Happy path writes everything once and the read-back verifies every write | PASS | 15 |
...
| Execution | `second_callout_same_day` | A second call-out later in the day keeps the first one in effect (never hands jobs back to Marco) | PASS | 33 |
...
| Execution | `calendar_transient` | Calendar 5xx errors are retried; each job still booked exactly once | PASS | 29 |
| Execution | `crash_after_calendar_write` | Crash right after a Calendar write: resume finishes without double-booking | PASS | 16 |
| Execution | `crash_after_email` | Crash right after sending an email: resume does not email the customer twice | PASS | 16 |
...
| Execution | `verification_catches_tampering` | Verification is not a rubber stamp: it catches a booking deleted behind our back | PASS | 15 |
| Execution | `sheet_calendar_conflict` | Sheet and Calendar disagree: Calendar wins and the conflict is reported | PASS | 0 |
| Guardrails | `callout_to_plan` | Clear call-out from the tech themself turns into a plan awaiting approval (no writes yet) | PASS | 19 |
...
| Guardrails | `message_handled_once` | The same Slack message is only ever handled once (poller restarts, duplicates) | PASS | 15 |
