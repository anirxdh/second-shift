...
```mermaid
flowchart LR
  S[Slack #dispatch] -->|message| P[LLM: read message<br/>structured output]
...
|---|---|---|
| Is this message a call-out? Who, and which hours? | An LLM with strict structured output: OpenAI by default, Claude or Groq by config | Language is messy: "car broke down, in by noon" |
