...
const OUT = '/Users/anirudh/Desktop/MultiApp-Ai-Agent-hack/video/assets/footage3';
const WORK = `/Users/anirudh/Desktop/MultiApp-Ai-Agent-hack/video/record/work3_${NAME}`;
const W = 1920, H = 1080;
...
async function glide(page, sel, { click = true, dx = 0, dy = 0, wait = 600 } = {}) {
  const el = page.locator(sel).first();
  const b = await el.boundingBox();
  if (!b) throw new Error(`no box for ${sel}`);
  await page.mouse.move(b.x + b.width / 2 + dx, b.y + b.height / 2 + dy);
  await sleep(wait);
  if (click) { await page.mouse.down(); await sleep(110); await page.mouse.up(); }
}
...
(async () => {
  ...
  if (MODE === 'live') {
    ...
    await smoothScroll(page, '#plan-scroll', 99999, 6000); mark('plan panel scrolled to bottom');
    ...
    await glide(page, '#actionbar .approve[data-action="approve"]', { wait: 700 });
    ...
    await sleep(3000);
    await glide(page, '#scan-dialog form[method="dialog"] button');
    ...
    await sleep(1400);
    await page.evaluate(() => window.__raiseCursor && window.__raiseCursor());
    await glide(page, '#lab-grid [data-lab="calcrash"]', { wait: 700 });
    ...
    await sleep(1800);
    await glide(page, '#actionbar .approve.resume', { wait: 700 });
    mark('clicked Resume');
    await park(page, 330, 900);
  ...
  fs.writeFileSync(path.join(OUT, `${NAME}.markers.json`), JSON.stringify({ video: outFile, firstFrameEpoch: first, markers: mk }, null, 2));
